...
> **Zum Server-Beitritt (wichtig, gründlich untersucht):** Der Client selbst ist **in Ordnung** — der Beitritts-Handshake (`initConnect`) wird bei manchen Servern vom **DDoS-Schutz des Hosters** verworfen. Das ist **ohne Wine reproduzierbar**: ein **nativer** `curl -X POST …/client` mit `method=getConfiguration` liefert **HTTP 200**, aber mit `method=initConnect` läuft er ins **Timeout** (derselbe Server, dieselbe IP) — also droppt der Hoster gezielt den Join, nicht unser Linux-Client. Für einen sauberen Nachweis, dass der Client **in-game** funktioniert, einen **lokalen fx-Server** (läuft nativ auf Linux) starten und per `fivem://connect/127.0.0.1:30120` beitreten — localhost umgeht Steam, Anti-Cheat und DDoS-Schutz. *(Der Rebuild aus aktuellem `citizenfx/master` + Patches ist unter Wine instabil; es bleibt der stabile `Gogsi/fivem wine-win10`-Build empfohlen.)*
